...
public class LotDaoImpl implements LotDao {
    ...
    public List<Lot> findByStockAndYear(Stock stock, int year) {
        try {
            ...
            // ✅ SOLUTION NATIVE SQL - Utiliser une requête native
            String nativeSql = "SELECT l.* FROM lot l " +
                              "WHERE l.idstock = ? " +
                              "AND EXTRACT(YEAR FROM l.datereception) = ? " +
                              "ORDER BY l.datereception DESC";
            
            List<Lot> resultats = em.createNativeQuery(nativeSql, Lot.class)
                .setParameter(1, stock.getId())
                .setParameter(2, year)
                .getResultList();
            
            System.out.println("✅ " + resultats.size() + " lots trouvés avec SQL natif");
            
            // Forcer le chargement des articles pour chaque lot
            ...
            // ✅ FALLBACK : Utiliser la méthode de secours
    ...
    /**
     * ✅ Charge les articles d'un lot spécifique
     */
    public Lot findByIdWithArticles(Long id) {
        try {
            // D'abord récupérer le lot
            Lot lot = em.find(Lot.class, id);
            if (lot != null && lot.getArticles() != null) {
                // Forcer le chargement des articles
                lot.getArticles().size();
                System.out.println("✅ Lot " + lot.getCodeLot() + " chargé avec " + 
                    lot.getArticles().size() + " articles");
            }
            return lot;
        } catch (Exception e) {
            System.err.println("❌ Erreur findByIdWithArticles: " + e.getMessage());
            return null;
        }
    }
    ...
    public String generateCodeLot() {
        try {
            ...
            Long count = em.createQuery(
                "SELECT COUNT(l) FROM Lot l WHERE l.codeLot LIKE :prefix",
                Long.class
            )
